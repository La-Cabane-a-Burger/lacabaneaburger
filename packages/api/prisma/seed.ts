...
async function main() {
  ...
  const pontchateauStore = await prisma.store.create({
    data: {
      city: 'Pontchâteau',
      slug: 'ponchateau',
      phone: '+33725078373',
  ...
  const nantesStore = await prisma.store.create({
    data: {
      city: 'Nantes',
      slug: 'nantes',
      phone: '+33725078373',
  ...
  const morning = await prisma.opening.create({
    data: {
      ...
      store: {
        connect: { id: pontchateauStore.id },
      },
    },
  })
  ...
  const nordiqueSaladItem = await prisma.item.upsert({
  ...
  const montagnardBurgerItem = await prisma.item.upsert({
    where: { name: 'Le Montagnard' },
    update: {},
    create: {
      name: 'Le Montagnard',
      description: '',
        ...
        create: [
          {
            ingredient: {
              connectOrCreate: {
                where: { name: 'Steak Haché' },
                create: { name: 'Steak Haché', category: 'viande' },
              },
            },
          },
          {
            ingredient: {
              connectOrCreate: {
                where: { name: 'Oignons jaune confits' },
                create: {
                  name: 'Oignons jaune confits',
                  category: 'légumes',
                },
              },
            },
          },
          {
            ingredient: {
              connectOrCreate: {
                where: { name: 'Sauce miel-moutarde' },
                create: {
  ...
  console.log({ user, morning, afternoon, pontchateauStore, nordiqueSaladItem })
}
